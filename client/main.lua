...
local function RegisterStashTarget()
    ...
    if Config.UseTarget then
        exports['qb-target']:AddBoxZone(stashTargetBoxID, coords, 1.5, 1.5, {
            ...
            options = {{
                type = "client",
                event = "lux-mechanicjob:client:target:OpenStash",
                label = "Open Stash",
            }},
            distance = 2.0
        })
    ...
    local OpenMenu = {
        {
            ...
            params = {
                event = "lux-mechanicjob:client:UnattachVehicle",
            }
        }, {
            header = "Check Status",
            txt = "Check Vehicle Status",
            params = {
                event = "lux-mechanicjob:client:CheckStatus",
                args = {
                    number = 1,
                }
            }
            ...
            params = {
                event = "lux-mechanicjob:client:PartsMenu",
                args = {
                    number = 1,
                }
            }
            ...
            txt = "",
            params = {
                event = "lux-mechanicjob:client:target:CloseMenu",
            }
        },
    }
        ...
        for k,v in pairs(Config.ValuesLabels) do
            if math.ceil(VehicleStatus[plate][k]) ~= Config.MaxStatusValues[k] then
                ...
                vehicleMenu[#vehicleMenu+1] = {
                    ...
                    params = {
                        event = "lux-mechanicjob:client:PartMenu",
                        args = {
                            name = v,
                            parts = k
                        }
                ...
                vehicleMenu[#vehicleMenu+1] = {
                    ...
                    txt = "Status: " .. percentage .. ".0% / 100.0%",
                    params = {
                        event = "lux-mechanicjob:client:NoDamage",
                    }
                }
            end                               
        end
...
local function PartMenu(data)
    ...
    local TestMenu1 = {
        ...
        },
        {
            ...
            params = {
                event = "lux-mechanicjob:client:RepairPart",
                args = {
                    part = part,
                }
            }
...
local function VehicleList()
    ...
    for k,v in pairs(Config.Vehicles) do
        vehicleMenu[#vehicleMenu+1] = {
            ...
            txt = "Vehicle: "..v.."",
            params = {
                event = "lux-mechanicjob:client:SpawnListVehicle",
                args = {
                    headername = v,
                    spawnName = k
                }
    ...
    QBCore.Functions.TriggerCallback('qb-inventory:server:GetStashItems', function(StashItems)
        ...
        if hasitem and countitem >= PartData.costs then
            ...
            QBCore.Functions.Progressbar("repair_part", "Repairing " ..Config.ValuesLabels[part], math.random(5000, 10000), false, true, {
                ...
                TriggerServerEvent('qb-inventory:server:SaveStashItems', "boss_ottos", StashItems)
...
RegisterNetEvent("lux-mechanicjob:client:UnattachVehicle",function(data)
    UnattachVehicle()
end)

RegisterNetEvent("lux-mechanicjob:client:PartsMenu",function(data)
    PartsMenu()
end)

RegisterNetEvent("lux-mechanicjob:client:PartMenu",function(data)
    PartMenu(data)
end)

RegisterNetEvent("lux-mechanicjob:client:NoDamage",function(data)
    NoDamage()
end)

RegisterNetEvent("lux-mechanicjob:client:CheckStatus",function(data)
    CheckStatus()
end)

RegisterNetEvent("lux-mechanicjob:client:SpawnListVehicle",function(data)
    local vehicleSpawnName = data.spawnName
    SpawnListVehicle(vehicleSpawnName)
end)

RegisterNetEvent("lux-mechanicjob:client:RepairPart",function(data)
...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function()
    QBCore.Functions.GetPlayerData(function(PlayerData)
        ...
        if PlayerData.job.onduty then
            if PlayerData.job.name == "Ottos" then
                TriggerServerEvent("QBCore:Ottos:ToggleDuty")
            end
        end
    end)
...
RegisterNetEvent('lux-mechanicjob:client:target:OpenStash', function ()
...
RegisterNetEvent('lux-mechanicjob:client:target:CloseMenu', function () 
    DestroyVehiclePlateZone(ClosestPlate)
    RegisterVehiclePlateZone(ClosestPlate, Config.Plates[ClosestPlate])
        ...
        if PlayerJob.name == "ottos" then
            ...
            if onDuty then
                if isInsideStashZone then
                    wait = 0
                    if IsControlJustPressed(0, 38) then
                        TriggerEvent("lux-mechanicjob:client:target:OpenStash")
                    end
                end
